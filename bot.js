...
function startBotLogic(chatId) {
  ...
  try {
    ...
    botClient.on('error', (err) => {
      ...
      if (chatId && !connectionStatus.isConnected) {
...
async function showMainMenu(chatId) {
  ...
  const statusEmoji = connectionStatus.isConnected ? '🟢' : '🔴';
  const statusText = connectionStatus.isConnected ? 'متصل' : 'غير متصل';
...
bot.onText(/\/status/, async (msg) => {
  ...
  if (connectionStatus.isConnected && botClient) {
    const players = Object.keys(botClient.players || {}).filter(p => p !== botClient.username);
    bot.sendMessage(chatId, 
      `📊 **حالة السيرفر**
━━━━━━━━━━━━━━━━━━━━━
...
📝 **الأسماء:** ${players.join(', ') || 'لا أحد'}`);
    ...
    const errorMsg = connectionStatus.error ? `\n📝 الخطأ: ${connectionStatus.error}` : '';
...
bot.onText(/\/connect/, async (msg) => {
  ...
  const success = startBotLogic(chatId);
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'my_servers') {
    ...
    servers.forEach((id, index) => {
      ...
      const isConnected = connectionStatus.isConnected && botData.activeServer === id;
  ...
  if (data.startsWith('select_server_')) {
    ...
    const statusText = connectionStatus.isConnected && botData.activeServer === serverId ? '🟢 متصل' : '🔴 غير متصل';
    const errorText = connectionStatus.error && botData.activeServer === serverId ? `\n📝 الخطأ: ${connectionStatus.error}` : '';
...
اختر الإعداد الذي تريد تعديله:`,
  ...
  if (data.startsWith('connect_bot_')) {
    ...
    if (connectionStatus.isConnected && botData.activeServer === serverId) {
      return bot.sendMessage(chatId, '✅ **البوت متصل بالفعل!**');
    }
