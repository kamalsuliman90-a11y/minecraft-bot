...
// ===== دالة تسجيل الدخول بحساب Microsoft =====
async function microsoftAuth(email) {
  return new Promise((resolve, reject) => {
    // استخدام مكتبة bedrock-protocol للتسجيل
    const authflow = require('bedrock-protocol').authflow;
    const code = authflow.requestCode(email);
    resolve(code);
  });
}
...
function startBotLogic() {
  ...
  let username = server.username || 'Bot' + Math.floor(Math.random() * 1000);
  ...
  // إذا كان الوضع Online ونحتاج إيميل
  if (useOnline && server.microsoftEmail) {
    clientOptions.username = server.microsoftEmail;
  }
  ...
  try {
    ...
    botClient.on('error', (err) => {
      ...
      // إذا كان خطأ تسجيل دخول Online Mode
      if (err.message && err.message.includes('authentication')) {
        console.log('⚠️ فشل تسجيل الدخول بحساب Microsoft، جاري إعادة المحاولة...');
        setTimeout(() => {
          startBotLogic();
        }, 5000);
      }
    ...
    // معالجة طلب كود التأكيد لـ Online Mode
    botClient.on('auth_request', (data) => {
      console.log('🔑 مطلوب كود تأكيد من Microsoft');
      // إرسال الرابط للمستخدم
      if (data && data.url) {
        console.log(`🔗 رابط التأكيد: ${data.url}`);
        // حفظ الرابط لإرساله للمستخدم
        pendingAuth[server.host] = {
          url: data.url,
          code: data.code
        };
        // تخزين البيانات
        if (!server.pendingAuth) server.pendingAuth = {};
        server.pendingAuth.url = data.url;
        server.pendingAuth.code = data.code;
        saveData();
      }
    });
...
📝 **الأسماء:** ${players.join(', ') || 'لا أحد'}`);
    ...
    const sentMsg = await bot.sendMessage(chatId, '🔴 **البوت غير متصل**\nاستخدم `/connect` للتوصيل');
  }
});
...
bot.onText(/\/connect/, async (msg) => {
  ...
  // إذا كان الوضع Online ويحتاج إيميل
  if (server.onlineMode && !server.microsoftEmail) {
    return bot.sendMessage(chatId, '❌ **وضع Online مفعل لكن لم يتم إدخال الإيميل!**\n\n📝 استخدم الأمر:\n`/set email <إيميل Microsoft>`', { parse_mode: 'Markdown' });
  }

  const sentMsg = await bot.sendMessage(chatId, '🔄 **جاري الاتصال بالسيرفر...**');
  ...
  try {
    ...
    setTimeout(async () => {
      if (botClient) {
        ...
        await bot.sendMessage(chatId, '✅ **تم الاتصال بالسيرفر بنجاح!**');
        // العودة للقائمة
        await showMainMenu(chatId);
      } else {
        await bot.deleteMessage(chatId, sentMsg.message_id);
        await bot.sendMessage(chatId, '❌ **فشل الاتصال بالسيرفر**\nتأكد من الإعدادات.');
        await showMainMenu(chatId);
      }
    }, 5000);
  } catch (error) {
...
bot.onText(/\/disconnect/, async (msg) => {
  ...
  if (botClient) {
    botClient.close();
    botClient = null;
    await bot.sendMessage(chatId, '🔌 **تم فصل البوت من السيرفر**');
  } else {
    await bot.sendMessage(chatId, '❌ البوت غير متصل حالياً.');
  }
  await showMainMenu(chatId);
});
...
bot.on('callback_query', async (query) => {
  ...
  if (data === 'add_server') {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل عنوان السيرفر (IP):**');
    return;
  }
  ...
  if (data === 'my_servers') {
    ...
    if (servers.length === 0) {
      await bot.sendMessage(chatId, '📋 **لا يوجد سيرفرات مضاف**\n\n➕ استخدم زر "إضافة سيرفر" لإضافة سيرفر جديد.');
      await showMainMenu(chatId);
      return;
    }
  ...
  if (data.startsWith('select_server_')) {
    ...
    if (!botData.servers[serverId]) {
      await bot.sendMessage(chatId, '❌ السيرفر غير موجود.');
      await showMainMenu(chatId);
      return;
    }
    ...
    const isOnline = server.onlineMode || false;
    ...
    const options = {
      reply_markup: {
        inline_keyboard: [
          [
            { text: `🌐 الوضع: ${onlineStatus}`, callback_data: `toggle_online_${serverId}` }
          ],
          [
          ...
          [
            { text: `🧠 الذكاء: ${aiStatus}`, callback_data: `toggle_ai_${serverId}` }
          ],
          [
...
اختر الإعداد الذي تريد تعديله:`,
  ...
  // ===== تبديل وضع الاتصال Online/Offline =====
  if (data.startsWith('toggle_online_')) {
    const serverId = data.replace('toggle_online_', '');
    const server = botData.servers[serverId];
    if (server) {
      server.onlineMode = !server.onlineMode;
      if (server.onlineMode) {
        // طلب إيميل Microsoft
        botData.tempData[userId] = { step: 'awaiting_email', serverId: serverId };
        saveData();
        await bot.sendMessage(chatId, `🌐 **تم تفعيل وضع Online!**

📝 أرسل إيميل حساب Microsoft الخاص بالبوت:

✏️ مثال: \`bot@outlook.com\`

🔗 بعد إدخال الإيميل، ستحتاج لتأكيد الدخول عبر الرابط.`);
        return;
      } else {
        // إيقاف Online Mode
        server.microsoftEmail = '';
        saveData();
        await bot.sendMessage(chatId, '🔴 **تم تعطيل وضع Online**\nالآن البوت يعمل بوضع Offline.');
        // العودة لإعدادات السيرفر
        await bot.emit('callback_query', {
          id: query.id,
          message: query.message,
          data: `select_server_${serverId}`
        });
      }
    }
    return;
  ...
  if (data.startsWith('edit_username_')) {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل الاسم الجديد للبوت داخل اللعبة:**');
    return;
  }
  ...
  if (data.startsWith('edit_callname_')) {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل اسم النداء الجديد (الكلمة المنادى بها):**');
    return;
  }
  ...
  if (data.startsWith('toggle_ai_')) {
    ...
    if (server) {
      ...
      await bot.sendMessage(chatId, `🧠 **تم تغيير حالة الذكاء في السيرفر إلى:** ${status}`);
      // العودة لإعدادات السيرفر
      await bot.emit('callback_query', {
        id: query.id,
        message: query.message,
...
اختر الرسالة التي تريد تعديلها:`,
  ...
  if (data.startsWith('edit_msg_')) {
    ...
    await bot.sendMessage(chatId, `📝 **أرسل ${msgNames[msgType] || 'الرسالة الجديدة'}:**`);
    return;
  }
...
اختر الإعداد الذي تريد تعديله:`,
  ...
  if (data.startsWith('toggle_movement_')) {
    ...
    if (server) {
      ...
      await bot.sendMessage(chatId, `🚶 **تم تغيير حالة الحركة إلى:** ${status}`);
      await bot.emit('callback_query', {
        id: query.id,
        message: query.message,
        data: `movement_settings_${serverId}`
      });
    }
    return;
  }
  ...
  if (data.startsWith('edit_radius_')) {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل نصف قطر الحركة الجديد (رقم):**');
    return;
  }
  ...
  if (data.startsWith('edit_speed_')) {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل سرعة الحركة الجديدة (بالثواني):**');
    return;
  }
...
اختر الإعداد الذي تريد تعديله:`,
  ...
  if (data.startsWith('edit_adhkar_time_')) {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل وقت الأذكار الجديد (بالدقائق):**');
    return;
  }
  ...
  if (data.startsWith('edit_sim_time_')) {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل وقت المحاكاة الجديد (بالدقائق):**');
    return;
  }
...
اختر الإعداد الذي تريد تعديله:`,
  ...
  if (data.startsWith('edit_sim_msgs_')) {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل رسائل المحاكاة (كل رسالة في سطر جديد):**');
    return;
  }
  ...
  if (data.startsWith('edit_adhkar_list_')) {
    ...
    saveData();
    await bot.sendMessage(chatId, '📝 **أرسل الأذكار (كل ذكر في سطر جديد):**');
    return;
  }
  ...
  if (data === 'connect_bot') {
    const server = getActiveServer();
    if (!server) {
      await bot.sendMessage(chatId, '❌ لا يوجد سيرفر نشط.');
      await showMainMenu(chatId);
      return;
    }
    if (!server.host) {
      await bot.sendMessage(chatId, '❌ السيرفر لا يحتوي على عنوان.');
      await showMainMenu(chatId);
      return;
    }
    ...
    // إذا كان الوضع Online ويحتاج إيميل
    if (server.onlineMode && !server.microsoftEmail) {
      await bot.sendMessage(chatId, '❌ **وضع Online مفعل لكن لم يتم إدخال الإيميل!**\n\n📝 استخدم الأمر:\n`/set email <إيميل Microsoft>`', { parse_mode: 'Markdown' });
      await bot.emit('callback_query', {
        id: query.id,
        message: query.message,
        data: `select_server_${botData.activeServer}`
      });
      return;
    }
    ...
    try {
      ...
      setTimeout(async () => {
        if (botClient) {
          ...
          // التحقق من وجود طلب تأكيد
          const server = getActiveServer();
          if (server && server.pendingAuth && server.pendingAuth.url) {
            await bot.sendMessage(chatId, `🔑 **مطلوب تأكيد الدخول!**

📌 افتح الرابط التالي وأدخل الكود:

🔗 ${server.pendingAuth.url}

📝 **الكود:** \`${server.pendingAuth.code}\`

⚠️ بعد التأكيد، سيدخل البوت تلقائياً.`);
          } else {
            await bot.sendMessage(chatId, '❌ **فشل الاتصال بالسيرفر**\nتأكد من الإعدادات.');
          }
        }
...
📝 **الأسماء:** ${players.join(', ') || 'لا أحد'}`);
      ...
      await bot.sendMessage(chatId, '🔴 **البوت غير متصل**');
    }
    await bot.emit('callback_query', {
      id: query.id,
      message: query.message,
...
bot.on('message', async (msg) => {
  ...
  if (botData.tempData && botData.tempData[userId]) {
    ...
    if (step.step === 'awaiting_host') {
      ...
      saveData();
      await bot.sendMessage(chatId, '📝 **أرسل المنفذ (Port):**');
      return;
    }
    ...
    if (step.step === 'awaiting_port') {
      ...
      if (isNaN(port) || port < 1 || port > 65535) {
        await bot.sendMessage(chatId, '❌ **منفذ غير صحيح.** أرسل رقماً بين 1 و 65535:');
        return;
      }
...
🌐 **وضع الاتصال:** 🔴 Offline (افتراضي)
👤 **اسم البوت:** ${botData.servers[serverId].username}
📢 **اسم النداء:** ${botData.servers[serverId].botCallName}

🔌 استخدم /connect لتوصيل البوت
🌐 لتغيير وضع الاتصال، اذهب لإعدادات السيرفر`);
      
      await showMainMenu(chatId);
      return;
    }

    // ===== انتظار إيميل Microsoft =====
    if (step.step === 'awaiting_email' && step.serverId) {
      const server = botData.servers[step.serverId];
      if (server) {
        server.microsoftEmail = text;
        saveData();
        
        // محاولة بدء عملية التسجيل
        try {
          await bot.sendMessage(chatId, `📧 **تم حفظ الإيميل:** ${text}

🔄 جاري محاولة تسجيل الدخول بحساب Microsoft...`);

          // محاولة الاتصال للحصول على رابط التأكيد
          startBotLogic();
          
          setTimeout(async () => {
            const updatedServer = botData.servers[step.serverId];
            if (updatedServer && updatedServer.pendingAuth && updatedServer.pendingAuth.url) {
              await bot.sendMessage(chatId, `🔑 **مطلوب تأكيد الدخول!**

📌 افتح الرابط التالي وأدخل الكود:

🔗 ${updatedServer.pendingAuth.url}

📝 **الكود:** \`${updatedServer.pendingAuth.code}\`

⚠️ بعد التأكيد، سيدخل البوت تلقائياً.`);
            } else {
              await bot.sendMessage(chatId, `⚠️ **لم يتم استلام رابط التأكيد بعد.**

🔄 حاول استخدام الأمر /connect يدوياً.`);
            }
          }, 5000);
        ...
        await bot.emit('callback_query', {
          ...
          data: `select_server_${step.serverId}`
        });
      }
      return;
    }
    ...
    if (step.step === 'edit_username' && step.serverId) {
      ...
      if (server) {
        ...
        saveData();
        await bot.sendMessage(chatId, `✅ **تم تحديث اسم البوت إلى:** ${text}`);
        delete botData.tempData[userId];
        saveData();
        await bot.emit('callback_query', {
          id: 'temp',
    ...
    if (step.step === 'edit_callname' && step.serverId) {
      const server = botData.servers[step.ser
